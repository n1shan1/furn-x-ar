...
Introducing our augmented reality application that allows you to visualize furniture in real time, bringing your interior design visions to life. With our app, you can easily preview furniture in your own space, adjust size and placement, and get a real feel for how it will look before making any changes. Whether you're an interior designer, architect, or simply someone who enjoys experimenting with different design ideas, our app offers a seamless and immersive experience that allows you to unleash your creativity and explore endless possibilities. 
...
To tackle the same, we have designed an AR-based application that helps the user to view the particular selected furniture to be viewed in real-time in actual space, giving the user a virtual experience of the actual existence of the furniture in the area, allowing the user to have an almost accurate idea about the existence of the furniture in the plane.
...
The application utilizes planar detection using the Vuforia Engine. It uses marker-less panel detection, (i.e.) the 3d models can be projected on any kind of planar surface.

The 3D models are being rendered in real-time using C# scripting.
...
The application is based on Unity and is native to Android Operating System. It makes use of the Vuforia engine for ground plane detection, markerless and marker-based. The overall application is scripted in C# language in unity.
...
At this point the application lacked the feature of moving, resizing, or rotating the 3D models. Along with that, the scaling issue persisted where the model was not shown at full scale according to the actual environment in which the model was being rendered.

These issues were fixed in this stage allowing the user to freely move the 3D model along the plane, rotate it according to the requirement and resize it. The scalability issue was almost fixed at this stage and required more work.
...
The only thing which isn't functional is the "BUY NOW" option, as we haven't implemented the same, and we are planning to implement the same real soon.
